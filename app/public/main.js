...
function getHTMLForTrips(trips, past) {
    ...
    for (let trip of trips) {
        ...
        let port = trip.direction === "outbound" ? "Vieques" : "Ceiba"

        html += "<div class='trip'>"
            html += "<div><span class='title'>Time</span>" + dayjs(trip.date).format("MM/DD hh:mma") + "</div>"
            html += "<div><span class='title'>Port</span>" + port + "</div>"
            html += "<div><span class='title'>Vessel</span>" + trip.vessel + "</div>"
            if (trip.timeInMotion) html += "<div><span class='title'>Departure</span>" + dayjs(trip.timeInMotion).tz("America/Puerto_Rico").format("hh:mma") + "</div>"
        html += "</div>"
    }
    return html
}
...
function updateMapMarkers(boats) {
    ...
    for (let boat of boats) {
        ...
        boatListHTML += `<div id="${boat.vesselId}-item" class="boat-item ${boat.vesselColor}">`
            ...
            boatListHTML += `<span>Desde ${dayjs(boat.timestamp).tz("America/Puerto_Rico").format("MM/DD hh:mma")}`
        boatListHTML += "</div>"
    ...
    for (let boat of boats) {
        ...
        if (Date.now() - boat.timestamp > (1000*60*15)) {
            $(`#${boat.vesselId}-boat`).css("opacity", "0.4")
            $(`#${boat.vesselId}-item`).css("opacity", "0.6")
        }
    }
}
